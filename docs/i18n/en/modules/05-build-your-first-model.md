...
This module walks through every step of building a real machine learning model from raw data to a saved, evaluated model file.

## Quick Review Links

- ML model fundamentals: [Module 01](01-machine-learning-basics.md)
- Azure ML platform flow: [Module 02](02-azure-ml-overview.md)
- Deploy and score the model: [Module 06](06-deploy-and-score.md)
...
- **Handle missing values**: if cells are blank, either fill them or remove those rows.
    - Fill with **mean/median** for numeric columns.
    - Fill with **mode** for categorical columns.
    - Remove rows only when missing data is small.
- **Encode categories**: algorithms need numbers, so text values are converted (e.g., "region" → 0, 1, 2).
- **Scale numbers**: normalize or standardize features so one large-range feature does not dominate learning.
...
Simple analogy: training questions help you learn; test questions check if you truly learned.
...
A **metric** is a number that tells how good or bad the model is.
Different tasks use different metrics.

### Regression Metrics (when target is a number)

- **MAE** — Mean Absolute Error. Average prediction error in real units.
    - Example: MAE = 3 means predictions are off by about 3 units on average.
- **RMSE** — Root Mean Squared Error. Similar to MAE but punishes big mistakes more strongly.
- **R²** — How much of the target variation the model explains (0 to 1, higher is better).
...
- **Overfitting**: training score is very high, test score is much lower. The model memorized training data and does not generalize. Fix: simplify the model, add regularization (a method to reduce memorization), get more data.

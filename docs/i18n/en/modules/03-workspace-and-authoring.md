...
Think of the workspace as your project home folder in the cloud. Instead of files spread across many laptops, your team uses one organized place.
...
- **Serverless compute**: on-demand machines that start automatically for jobs and stop when done.
...
In simple terms, each job is one experiment attempt.
...
Why this matters: without pinned versions, code that works today can fail later after package updates.

![Azure ML environment taxonomy](../assets/img/azure-ml-environment-taxonomy.svg)

### Model Registry

The model registry stores trained model files with full metadata: which dataset and job produced them, what evaluation metrics they achieved, and which environment they were trained in. You can promote a model from development to production directly from the registry.
...
- **HTTP API** means one program can send data to another program over the web and get a response.
...
1. Write code in one cell.
2. Run it.
3. Read the output below.
4. Update the next cell.

### AutoML

Automated ML tests dozens of algorithms and model settings automatically. You provide a dataset and a target column. Azure ML trains and evaluates many models in parallel and returns the best one. AutoML is useful when you want a strong baseline quickly or when you want to compare automated results against your manual approach.

This is useful when you are still learning and want to see what "good baseline performance" looks like.
...
This helps students understand the process before writing full scripts.
...
| **Output files** | Files saved by a job: model file, metrics CSV, logs. |
| **Project history** | A full record of which data, code, and environment produced a model. |
